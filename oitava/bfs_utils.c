...
typedef struct {
    QueueNodeList *front;
    QueueNodeList *back;
    QueueNodeList *freeList; // List of free nodes for reuse
    int size;
} Queue;
...
QueueNodeList* getFreeNode(Queue *queue) {
    if (queue->freeList) {
        QueueNodeList *node = queue->freeList;
        queue->freeList = queue->freeList->next;
        return node;
    }
    return (QueueNodeList *)malloc(sizeof(QueueNodeList));
}

void returnFreeNode(Queue *queue, QueueNodeList *node) {
    node->next = queue->freeList;
    queue->freeList = node;
}

void enqueue(Queue *queue, int row, int col, int distance) {
    QueueNodeList *newNode = getFreeNode(queue);
    if (!newNode) {
        return;
    }
...
QueueNode dequeue(Queue *queue) {
    ...
    if (queue->front) {
        ...
        returnFreeNode(queue, frontNode);
        queue->size--;
    }
    return node;
}
